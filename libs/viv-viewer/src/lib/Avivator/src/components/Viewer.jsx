...
import {defaultPlateZoom, initialLayerZoom, zoomTextVisibilities, tooltipBackgroundColor} from '../../../../../../layers/constants';
...
const Viewer = (props) => {
  ...
  const regenerateTextLayers = (zoom) => {
    setTextLayers([
      generatePlateText(props.processedData.plateOverlayData, props.overlayVisibilities["All Overlays"] && props.overlayVisibilities["Text"] && zoom < defaultPlateZoom),
      generateText(props.processedData.wellOverlayData, props.processedData.cellSize, props.overlayVisibilities["All Overlays"] && props.overlayVisibilities["Text"] && zoom > zoomTextVisibilities["well_location"] && zoom < -3, "well_location")
    ]);
  };
  
  const regeneratePlateHeatmap = (zoom) => {
    setPolygonLayer([
      generatePolygons(props.processedData.plateOverlayData, props.overlayVisibilities["All Overlays"] && props.overlayVisibilities["Plate Heatmap"] && zoom<plateZoom, props.heatmapOpacity)
    ]);
  };
  const regenerateWellHeatmap = (zoom) => {
    setHeatmapLayer([
      generateHeatmap(props.processedData.wellOverlayData, props.processedData.cellSize, props.overlayVisibilities["All Overlays"] && props.overlayVisibilities["Well Heatmap"] && zoom < -3 && zoom >= plateZoom, 1, props.heatmapOpacity, props.heatmapId)
    ])
  };
...
useEffect(() => {
  Promise.all(props.urls.map((url) => createLoader(url))).then((values) => setLoaders(values.map((value) => value.data)));
...
useEffect(()=>{
  const updatedPlateZoom=props.overlayVisibilities["Well Heatmap When Zoomed Out"] ? -Infinity : defaultPlateZoom;
...
const handleZoom = (vws) => {
  ...
  if ((vws.viewState.zoom < plateZoom && vws.oldViewState.zoom >= plateZoom) ||
  ...
  if (
    (vws.viewState.zoom >= defaultPlateZoom && vws.oldViewState.zoom < defaultPlateZoom) ||
    (vws.viewState.zoom < defaultPlateZoom && vws.oldViewState.zoom >= defaultPlateZoom) ||
    (vws.viewState.zoom >= zoomTextVisibilities["well_location"] && vws.oldViewState.zoom < zoomTextVisibilities["well_location"]) ||
    (vws.viewState.zoom < zoomTextVisibilities["well_location"] && vws.oldViewState.zoom >= zoomTextVisibilities["well_location"])
  ) {
    regenerateTextLayers(vws.viewState.zoom);
  }
...
const deckRef = React.useRef(null);
  return use3d ? (
    ...
    loaders.length > 0 ? (<PictureInPictureViewer
    gridLoaders={{
      loaders,
      spacingX: 100000,
      spacingY: 100000,
    ...
    deckProps={{
      ...
      ref: deckRef,
              getTooltip: (arg) => {
                ...
                useViewerStore.setState({pixelValues: getPixelValues(pickInfos?.[1])});
                const object = arg.object;
                return (
                  props.overlayVisibilities["Tooltip"] && object && {
                    html:
                      zoomState >= plateZoom
                        ? `${
