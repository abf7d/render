...
class ImageMenuWebComponent extends HTMLElement {
    ...
    selectChannel(sliderIndex: number, channelIndex: number) {
        // @ts-ignore
        store.getState().onSelectionChanges[sliderIndex]({target:{value:"Channel " + channelIndex}});
    }
    constructor(){
        super()
        const shadow=this.attachShadow({mode:"open"});

        const pixelValuesContainer=shadow.appendChild(document.createElement('div'));
        ...
        useViewerStore.subscribe((currentState: any, previousState: any)=>{
            if(JSON.stringify(currentState.pixelValues)!==JSON.stringify(previousState.pixelValues)) {
                pixelValuesContainer.innerHTML=currentState.pixelValues.slice(0,numberOfChannels).map((value:[])=>`<div style="margin-top:91px;">${value}</div>`).join("");
            }
        });
            ...
            if(JSON.stringify(currentState.colors)!==JSON.stringify(previousState.colors)) {
                ...
                const render = () => {
                    menuContainer.innerHTML=`
                        ${currentState.colors.map((color: number[], index: number)=>`
                        ...
                        ></multi-range-slider></div>
                               ...
                               <select style="width:30%;margin-left:110px;" onchange="this.getRootNode().host.selectChannel(${index}, this.selectedIndex)" class="form-select">
                                ${channelOptions.map((channelOption, channelOptionIndex)=>`
                                <option ${index===channelOptionIndex ? "selected" : ""}>${channelOptions[channelOptionIndex]}</option>
                                `)}
                            </select>
                        ...
                        >
                            ${colorOptions.map((colorOption: any)=>`<option ${JSON.stringify(colorOption[1]) === JSON.stringify(color) ? "selected" : ""} value="${colorOption[1]}">${colorOption[0]}</option>`).join("")}
                        </select>
                        </div>
                            </div>
                        `).join("")}
                ...
                if (!initialized) {
                    const unsubscribeViewerStore = useViewerStore.subscribe((currentViewerStoreState: any, previousViewerStoreState: any)=>{
