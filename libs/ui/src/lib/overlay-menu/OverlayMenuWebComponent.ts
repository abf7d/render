...
class OverlayMenuWebComponent extends HTMLElement {
    ...
    handleClick(arg: any){
        store.setState({overlayVisibilities: {...store.getState().overlayVisibilities, [arg.value]: arg.checked}})
    }
    ...
    constructor() {
        ...
        store.subscribe((currentState: any, previousState: any)=>{
            if(JSON.stringify(currentState.heatmapIds)!==JSON.stringify(previousState.heatmapIds)) {
                ...
                <div style="padding:10px;margin-left:4px;margin-right:8px;margin-top:19px;">
                    ...
                    >
                        ${currentState.heatmapIds.map((item: any)=>`<option value="${item.value}">${item.label}</option>`).join("")}
                    </select>
                    ${Object.entries(state.overlayVisibilities).map(([key, value])=>`
                        <div class="form-check" style="margin-bottom: 8px;">
                            <input class="form-check-input" type="checkbox" ${value ? "checked" : ""} value=${`"${key}"`} onclick="this.getRootNode().host.handleClick(this)">
                    ...
                    <input style="width:100%;" type="range" min=0 max=1 value=${state.heatmapOpacity} step=0.01 oninput='this.getRootNode().host.setHeatmapOpacity(this.value)'>
                    </div>
                `;
