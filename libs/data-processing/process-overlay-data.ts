export const processOverlayData: (arg: {
  overlayData: any[];
  baseChemicalUrl: string;
  cellSize: number;
  plateSpacingX: number;
  plateSpacingY: number;
  platesPerRow: number;
  wellsPerRow: number;
  wellsPerColumn: number;
}) => any = ({
  overlayData,
  baseChemicalUrl,
  cellSize,
  ...
  for (let i = 0; i < numberOfPlates; i++) {
    ...
    for (let j = 0; j < numberOfWells; j++) {
      ...
      wellOverlayData[i * numberOfWells + j].drug_name =
        drugNameData === '' ? 'Drug name unavailable' : drugNameData;
      ...
      wellOverlayData[i * numberOfWells + j].fillColors = overlayData[
        i
      ].gridCellLayers.reduce(
        (acc: any, entry: any) => ({
          ...acc,
          [entry.id]: entry.data[j].fillColor,
        }),
        {}
      );
    }
